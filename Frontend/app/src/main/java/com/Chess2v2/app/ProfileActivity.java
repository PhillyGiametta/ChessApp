...
public class ProfileActivity extends AppCompatActivity {
    ...
    private void loadProfile() {
        String username = ChessApplication.getInstance().getUserName();
        if (username != null) {
    ...
    private void listProfiles() {
        apiService.listProfiles().enqueue(new Callback<List<UserResponse>>() {
            @Override
            public void onResponse(Call<List<UserResponse>> call, retrofit2.Response<List<UserResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
